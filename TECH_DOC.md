...
> 卡片布局口径（2026-08-16 优化）：卡片高度自适应页面可用空间（上限 520，
> 无界约束回退 320），提高屏幕利用率；反面例句固定在释义区下方的独立展示区、
> **始终可见**（释义较多时仅释义区内部滚动），避免例句被挤出首屏而误以为
> 单词没有例句；例句过长时可点按展开/收起（展开态超出部分在例句区内滚动，
> 不撑破卡片）。实现：`lib/features/learn/widgets/session_card.dart`。
...
| 例句 | Tatoeba **Detailed** 导出（自带作者用户名）过滤：句长 3–18 词、包含目标词**原型**（词边界、忽略大小写；屈折形态召回优化列为后续增强）、句子用词不超出高中词汇集（内容词须在 gk 集内或 ECDICT `bnc` 排名 ≤ 30000，功能词白名单豁免）；**候选排序（2026-08-15 调整）**：长度偏好 6–12 词优先（4–5 / 13–18 次之，3 词仅兜底），≤6 词且以人称代词+be 开头的套话短句加 2 分，按（得分, 词数, sentence_id）取前 2；入库句子 ID、`source=Tatoeba`、`attribution=作者用户名` |
...
> 候选池下 6–9 词占比升至 99.3%，套话短句占比由 23.5% 降至 4.4%，例句总数与
> 覆盖率（99.6%）不变。规则常量见 `common.py`，实现在 `select_examples`。

> **音标展示与入库规范（2026-08-15 按产品反馈确认；同日决策：暂不更换主源）**：
> 无论音标来自 ipa-dict 还是 ECDICT 兜底，入库与展示前统一执行 `normalize_ipa`
> 归一化（App 显示层 `lib/domain/models/ipa_display.dart` 与管线
> `tools/content_pipeline/pipeline/build_wordbook.py` 各一份实现，口径以此为准）。
> 归一化内容为一次性"美式学习者惯例显示表"（目标用户为高中生，主流学习者词典
> 用普通 `r`/`l`；美音发音口径不变）：
>
> | 字符 | 处理 |
> |---|---|
> | `ɹ` U+0279（turned r，观感"翻转的 r"） | → `r` |
> | `ɫ` U+026B（暗 l，观感"l 带中划线"） | → `l`（亮/暗 l 不再区分） |
> | `ә` U+04D9（西里尔 schwa，ECDICT 编码污染，v1.0 实测 23 词） | → `ə` U+0259 |
> | `є` U+0454（西里尔 ye，同上，实测 1 词） | → `e` |
> | 音标首尾 `/`（ipa-dict 原始自带） | 去除，展示层统一包一层 |
> | `ɡ`、`ɝ`、`ɚ` 等其余严格 IPA 符号 | **保留**（如 ɡ 为剑桥美音词典官方写法） |
>
> 空音标（ECDICT 兜底缺失，v1.0 实测 4 词）在 UI 隐藏，不展示 `//`。
>
> 决策背景（2026-08-15 与产品方确认）：免费可用音标源（CMUdict/ipa-dict/
> Wiktionary/ECDICT）均为严格 IPA 或惯例混杂，学习者惯例音标为商业词典私有
> 数据；项目非商业化、无付费来源，暂不换源，以归一化表解决显示惯例问题。
